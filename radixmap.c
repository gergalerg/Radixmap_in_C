...
void radix_sort(short offset, uint64_t max, 
...
void RadixMap_printer(RadixMap *map)
{
	int end = map->end;
	int i = 0;
	for ( ; i < end; i++) {
		printf("key = %u\n", map->contents[i].data.key);
		printf("value = %u\n", map->contents[i].data.value);
	}
}